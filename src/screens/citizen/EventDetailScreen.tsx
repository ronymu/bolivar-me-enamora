...
import MapView, { Marker, PROVIDER_GOOGLE } from "react-native-maps"; // Keep for map functionality
import { ArrowLeft, Heart, MapPin, Share2, Calendar, Phone, MessageCircle, User } from "lucide-react-native";
...
function openNavigationChoice(lat: number, lng: number) {
  // Per user feedback, using the 'search' URL for Google Maps. It's more universal and robust
  // for placing a pin at the exact destination coordinates, from which the user can start navigation.
  // The 'dir' action was reported to be unreliable on some devices.
...
export default function EventDetailScreen({ navigation, route }: Props) {
  ...
  const event = useMemo(() => getById(eventId), [eventId, getById]);
  const organizer = useMemo(() => getOrganizer(event), [event]);
  const [signedOrganizerAvatar, setSignedOrganizerAvatar] = useState<string | null>(null);
  ...
  // Effect to sign the organizer's avatar URL.
  // This is necessary because the avatars are in a private Supabase bucket.
  useEffect(() => {
    let alive = true;
    const rawAvatarUrl = organizer.avatar_url;

    if (rawAvatarUrl) {
      // The URL is signed for 1 hour (3600 seconds).
      resolveSignedUrl(rawAvatarUrl, 3600, { bucket: "avatars" })
        .then((url) => alive && setSignedOrganizerAvatar(url))
        .catch(() => alive && setSignedOrganizerAvatar(null));
    } else {
      setSignedOrganizerAvatar(null);
    }
  ...
  // Data is now strongly typed from the event object.
          ...
          <View style={styles.card}>
            ...
            <View style={styles.organizerCard}>
              <View style={styles.organizerAvatar}>
                {signedOrganizerAvatar ? (
                  <Image source={{ uri: signedOrganizerAvatar }} style={styles.organizerAvatarImage} contentFit="cover" />
                ) : (
              ...
              <View style={styles.organizerActions}>
                {organizer.phone && (
                  <Pressable
                    ...
                    onPress={() => Linking.openURL(`tel:${organizer.phone}`).catch(() => {})}
                    hitSlop={10}
                  >
